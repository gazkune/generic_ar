...
# For DELTA = 0, sequences may turn to long to process for a LSTM (e.g. kasterenB 1155)
# Use MAX_ABSOLUTE_SEQUENCE_LENGTH to avoid such sequences
...
def prepare_embeddings(df, action_dict, activity_dict, temporal_dict, activity_to_int, delta = 0):
    ...
    if delta == 0:
        ...
        for index in df.index:        
            ...
            if current_activity != df.loc[index, 'activity'] or len(actionsdf) >= MAX_ABSOLUTE_SEQUENCE_LENGTH - 1:
                ...
                if DAYTIME == 'with_time':
                    # First of all calculate the day period
                    # Using 'first' and 'last' we have to add the time period of a day at the beginning of 'actionsdf'
                    first_period = obtain_day_period(df.loc[first_index, 'timestamp'])
                    last_period = obtain_day_period(df.loc[last_index, 'timestamp'])
                    # Create an auxiliary DataFrame to store the slice between 'first' and 'last'
                    auxdf = df[first_index:last_index]
                    action_seq_period = ''
